...
1.  **Do not set Longhill's admission number without calibrating this analysis on the council's own records.** The question is not a fine judgement between adjacent options; it is whether the school supports three forms of entry or five, and published data cannot distinguish those. The work is a day's analysis on records the admissions process already produces.

2.  **Publish the evidence base.** Nine aggregate tables, none disclosive, most a single query, set out at @sec-data-requests. Publishing them alongside the allocation factsheets each March would let every party to these debates argue from the same evidence, and would cost a day of officer time a year.

3.  **Publish an intake-adjusted attainment measure in the admissions guide**, alongside the raw one, with a sentence explaining what a headline grade mostly reflects. The guide already tables roll, preferences and admission numbers for every school. This is a column, not a new document, and it is the only lever identified anywhere in this report that acts on demand rather than shuffling supply.

4.  **Stop treating catchment priority as an instrument where it is inert.** At five of the city's schools it allocates nothing. Any proposal resting on a redraw should say explicitly whether it expects to work through priority, which it cannot at those schools, or through preference, which is slower, weaker and largely untested.

5.  **State which goal is being pursued when the two conflict**, and be honest that pursuing it costs something measurable on the other. A consultation presenting a redraw as good for disadvantaged children *and* good for an eastern school is describing a trade-off as though it were a win.
...
The arrangements set out a careful worked calculation for capping FSM places at a city average of 30% of PAN. After the narrowing, FSM allocations at Blatchington Mill and Dorothy Stringer fall to roughly **8% of PAN**, and Varndean to about **16%**.

The cap is therefore inert. What limits FSM admissions under the new arrangements is the size of the eligible pool, not the ceiling the council has written above it — and a reader of the arrangements would not guess that from the prominence the calculation is given.
:::
...
Across the six community schools it amounts to roughly **79 places city-wide**. It is aimed precisely at the four catchments where a family has no choice of school within their own area — which is a defensible thing to target, and a different thing from what the FSM criterion targeted.
...
What relocation does to Longhill's recruitment, on its own and combined with a redraw or a smaller admission number, is the subject of @sec-design-open --- and, with catchment terms fitted to what families ask for, of @sec-relocation-m5, where the answer changes.
...
**What a fill rate would look like instead can be bounded rather than asserted.** @sec-envelope shows this model recruiting roughly twice what the lower end of its own sweep suggests. Carry that through and a PAN of 150 in 2035 could plausibly fill somewhere in the high fifties rather than reaching its number in nine runs out of ten; a PAN of 120 might sit in the low seventies. If the true fill rates are anywhere near that, **the conclusion this section invites — that 120 to 150 is sustainable — does not follow.** A school at 57% of a 150 PAN is not a viable school; it is a smaller version of the same problem.
...
**Moving with redrawn catchments gains a few children.** Configuration E draws `r m5n("E", 2026)` in 2026 and `r m5n("E", 2035)` in 2035, against `r m5n("B", 2026)` and `r m5n("B", 2035)` for shrinking where it is --- about `r round(rl5$move_effect$move_and_redraw[rl5$move_effect$entry_year == 2030])` a year. The redraw wins back what the move lost, and a little more; fewer of Longhill's catchment leave the city under it, `r round(rl5$central$left_city_longhill[rl5$central$id == "E" & rl5$central$entry_year == 2030])` in 2030 against `r round(rl5$central$left_city_longhill[rl5$central$id == "B" & rl5$central$entry_year == 2030])` shrinking in place. Redrawing without moving costs children too (configuration F, `r m5n("F", 2035)` in 2035), because the designed map passes some of Longhill's hinterland to its neighbours.
...
The open model has Cardinal Newman recruiting only `r round(po$slack$intake[po$slack$name == "Cardinal Newman Catholic School"])` of its `r po$slack$pan[po$slack$name == "Cardinal Newman Catholic School"]` places. Reducing its admission number to 270 therefore binds on nothing, because the school never reaches 270 in the first place.

In reality Cardinal Newman fills every year and is the most oversubscribed school in the city. The open model under-predicts it because the only public signal of a faith school's popularity is its first-preference count, and that badly understates demand for a school which families know admits on religious criteria — many who would want a place never express the preference. The same weakness that makes @sec-choice cautious about attractiveness proxies bites hardest here.
...
- Longhill cannot sustain the PAN of 210 now in force into the 2030s: it reaches 90% of that number in only `r round(env$q1[["PAN 210"]][env$q1$entry_year == 2035])`% of runs by 2035, and that is on a model already biased in its favour. At the 240 it carried until 2026, `r round(env$q1[["PAN 240"]][env$q1$entry_year == 2035])`%.
...
- The 2024 catchment redraw does not alter it either, in the direction the open model can see (@sec-redraw). Whether it makes Longhill's position slightly better or slightly worse is beyond what published data can settle.
- Demand tracks published Attainment 8, not value-added.
- On the swept model, relocating to Elm Grove increases recruitment, in `r round(100 * mean(env$q3$gain[env$q3$entry_year == 2035] > 0))`% of runs by
  2035. With catchment terms fitted to preferences a move on its own does the opposite (@sec-relocation-m5), so this is listed here for what the envelope says, not as a conclusion that holds.

**What this analysis cannot settle**

- *Whether* relocation would help Longhill's roll at all. The swept model says it would. Fitted catchment terms say a move on its own would cost the school about `r abs(round(rl5$move_effect$move_only[rl5$move_effect$entry_year == 2030]))` children a year and a move with redrawn catchments would gain about `r round(rl5$move_effect$move_and_redraw[rl5$move_effect$entry_year == 2030])` (@sec-relocation-m5). What decides it is how strongly families would follow a redrawn boundary, which no data here can show.
...
- The model over-predicts Longhill by `r round(os$val$modelled[os$val$name == LH] - os$val$observed[os$val$name == LH])` children against published offers, so its figures are generous to the school and its relocation gain is an upper bound.
...
Prediction 1 quotes a range centred on the median of the sweep. For the reasons set out in @sec-envelope, there is good reason to think the true figure sits at or below the **bottom** of that range rather than in the middle of it — which would put Longhill's 2035 recruitment closer to 80 children than to 150.
...
1.  Longhill's natural recruitment in 2035 sits between `r round(env$q2$min[env$q2$entry_year == 2035 & env$q2$site_label == "Ovingdean"])` and `r round(env$q2$max[env$q2$entry_year == 2035 & env$q2$site_label == "Ovingdean"])` children under current catchments, centred near a median of `r round(env$q2$median[env$q2$entry_year == 2035 & env$q2$site_label == "Ovingdean"])` children.
2.  Relocating to Elm Grove without redrawing catchments does **not** increase Longhill's intake. On the swept model it rises, by a median of `r round(median(env$q3$gain[env$q3$entry_year == 2035]))` children a year by 2035; with catchment terms fitted to the council's catchment preference table it falls, by about `r abs(round(rl5$move_effect$move_only[rl5$move_effect$entry_year == 2035]))` (@sec-relocation-m5). The second is the better evidenced, and it is the prediction made here.
3.  The four wards joining in 2028 contribute fewer than 10 additional children a year to Longhill, under arrangements retaining a catchment priority at Peacehaven Community School.
