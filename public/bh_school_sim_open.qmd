...
The same seven configurations, re-run under that model --- the strategic view's calibrated model, M5 --- with the same admission numbers, the same designed maps and the same cohort projection. One thing does differ from the runs above: a child refused at a full school is re-offered a place the way a second preference would be --- at the other school of a paired catchment first, then in proportion to where the family's catchment puts its second preferences, and only at schools with room --- rather than in proportion to first preferences:
...
**Moving with redrawn catchments roughly breaks even.** Configuration E draws `r m5n("E", 2026)` in 2026 and `r m5n("E", 2035)` in 2035, against `r m5n("B", 2026)` and `r m5n("B", 2035)` for shrinking where it is. The redraw wins back what the move lost and little more. Redrawing without moving costs children too (configuration F, `r m5n("F", 2035)` in 2035), because the designed map passes some of Longhill's hinterland to its neighbours.
...
- *Whether* relocation would help Longhill's roll at all. The swept model says it would. Fitted catchment terms say a move on its own would cost the school about `r abs(round(rl5$move_effect$move_only[rl5$move_effect$entry_year == 2030]))` children a year and a move with redrawn catchments would roughly break even (@sec-relocation-m5). What decides it is how strongly families would follow a redrawn boundary, which no data here can show.
