...
**The location finding stands.** Nothing in this run touches it: relocation raises Longhill's intake at every decay value tested, on geography alone, with no attractiveness assumption anywhere in the calculation.
...
- Relocating to Elm Grove increases recruitment, in `r round(100 * mean(env$q3$gain[env$q3$entry_year == 2035] > 0))`% of runs by
  2035. The direction is robust; the size is not, because the model over-predicts Longhill to begin with.
...
2.  Relocating to Elm Grove without redrawing catchments *increases* Longhill's intake, by a median of `r round(median(env$q3$gain[env$q3$entry_year == 2035]))` children a year by 2035 on this model — which, given the model's known bias towards the school, should be read as an upper bound.
