...
That is `r fmt(nrow(env$envelope))` model runs. Where a single number is needed to draw a chart, the **central specification** is the midpoint of each range — $\beta$ = `r os$spec$beta`, $\gamma$ = `r os$spec$gamma`, attractiveness from published first preferences — chosen for legibility only, with the band from the sweep reported alongside.
...
Raw Attainment 8 explains **`r sprintf("%.0f%%", 100*os$choice_tbl$r2[os$choice_tbl$spec == "Attainment 8"])`** of the variation in how much Brighton families want a school. Value-added — the measure of what the school actually contributes once its intake is accounted for — explains **`r sprintf("%.0f%%", 100*os$choice_tbl$r2[os$choice_tbl$spec == "Value-added"])`**.
